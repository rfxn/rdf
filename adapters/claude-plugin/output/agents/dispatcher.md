...
You are invoked as a subagent by /rdf:r-build. You resolve the active plan,
...
inter-phase parallel batch managed by /rdf:r-build. In this case,
...
**(b) Ensure the scope guard is active.** `/rdf:r-build` normally installed
...
This is separate from /rdf:r-ship's sentinel — /rdf:r-ship provides a
