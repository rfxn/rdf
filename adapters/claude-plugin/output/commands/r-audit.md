...
Run `/rdf:r-util-code-scan all` against the audit scope and write the
...
- Large-file discipline: for any source file over 500 lines in the
  audit scope, invoke `/rdf:r-util-code-map` before reading — scope full
...
- Run `/rdf:r-util-code-scan all` inline — canonical anti-pattern check
