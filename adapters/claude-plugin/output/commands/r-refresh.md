...
Re-run the /rdf:r-init codebase analysis against current state:
...
Run `/rdf:r-util-code-scan all` and parse the per-pattern row counts from
...
(the snapshot from the prior refresh or /rdf:r-init run).
...
- ignore.md — refresh from profiles/core/reference/ignore-defaults.md
  ...
  append only new defaults under a `# Added by /rdf:r-refresh` heading
...
here — direct the user to `/rdf:r-util-code-scan <class>` for detail.
...
```
> **Warnings**
> - `MEMORY.md` at 185/200 lines — run `/rdf:r-util-mem-compact`
