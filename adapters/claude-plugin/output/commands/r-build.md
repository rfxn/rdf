...
`/rdf:r-build [N | N-M | --parallel] [--worktree] [--max N] [--consistency-warn-only "<reason>"]`
...
- Source `~/.rdf/state/rdf-bus.sh` and call `rdf_session_init`. If the helper
  is absent (helpers not yet bootstrapped — fresh plugin install before its
  first session restart), stop and tell the user: `/rdf:r-build` needs the
...
- Read `$plan_path` as the plan input from here forward. Validate
  ...
  trigger check as `/rdf:r-plan` Step 2.7: scan each phase's `**Test**`
  ...
  /rdf:r-plan Step 2.7 should have caught any schema violations before the
...
- If no argument: scan phases in order, target first with
  ...
  - If all phases are complete, report "All phases complete. Use
    /rdf:r-ship for release workflow." and stop
...
- Read `.rdf/governance/index.md`
  - If governance index does not exist, warn: "No governance found.
    Run /rdf:r-init to generate governance, or proceed without it."
...
- If PASS and more phases remain:
  ...
  > Next: Phase {N+1} — {description}. Run `/rdf:r-build` to continue.
- If PASS and all phases are complete:
  - If PLAN_PHASE_COUNT >= 3: the dispatcher runs end-of-plan sentinel
    ...
    > Run `/rdf:r-ship` to begin the release workflow.
...
- Parallel dispatch is additive — /rdf:r-build N (single phase) works unchanged
