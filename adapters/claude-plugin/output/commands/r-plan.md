...
```
/rdf:r-plan                          — auto-detect most recent spec in docs/specs/
/rdf:r-plan docs/specs/foo.md        — file path (contains / or ends with .md)
/rdf:r-plan https://github.com/...   — GitHub URL (starts with http/https)
/rdf:r-plan #42                      — issue shorthand (# + digits)
/rdf:r-plan --resume                 — resume interrupted plan (resolved via rdf_active_plan_path)
/rdf:r-plan --resume <path>          — set pointer to <path> and resume
/rdf:r-plan --full|--quick|--bugfix  — set the task-class tier (may combine with a spec input)
```
...
- Auto-detect: scan `docs/specs/` sorted by mtime descending, use
  ...
  delegation to `/rdf:r-spec`:
  ```
  "No spec found. Would you like to start with /rdf:r-spec to design
  one first? [Y/n]"
  ```
  If Y: run the `/rdf:r-spec` workflow inline, then continue into
...
plugin install before its first session restart), skip session init, treat
the tier as `full`, and resolve the plan from the committed `docs/plans/`
file directly; restart the session (plugin) or run `rdf deploy claude-code`
(checkout) to deliver them.*
...
- **Inherited tier.** If `/rdf:r-spec` already set a session tier (pointer present)
...
level resolved in Tier Selection; `/rdf:r-build` and the dispatcher read it to cap
...
Structured dependency list — required for all plans. `/rdf:r-build
...
(multiple phases running concurrently) is handled by `/rdf:r-build
...
the reviewer and `/rdf:r-build` apply the same rules and would reject the
...
3. Step 2.7 schema validation pass ran clean?
   ...
   Regression-case). Do not re-walk every rule — /rdf:r-build Section 1 is
...
- Do NOT re-run the spec design phases — use `/rdf:r-spec` for that
...
> Run `/rdf:r-build` to begin execution, or `/rdf:r-build 3` for a specific phase.
```
...
- **Never write implementation code** — the planner produces plans
  only. Implementation is for `/rdf:r-build`.
