...
/rdf:r-mode [name]          Switch to the named mode
/rdf:r-mode                 Show current mode and available modes
...
4. The mode context is now active for this session. All subsequent
   agent dispatches (`/rdf:r-build`, `/rdf:r-verify`, `/rdf:r-review`, etc.) will
...
| Agent | Mode Effect |
|-------|-------------|
| **Spec** (`/rdf:r-spec`) | *Research methodology and brainstorming focus change* |
...
```
/rdf:r-mode security        # Switch to security assessment mode
/rdf:r-mode performance     # Switch to performance audit mode
/rdf:r-mode migration       # Switch to migration planning mode
/rdf:r-mode dev             # Switch back to default development mode
/rdf:r-mode                 # Show current mode and available modes
```
