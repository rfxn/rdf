...
/rdf:r-spec, /rdf:r-plan, /rdf:r-build, and /rdf:r-ship independently.

## Invocation

/rdf:r-vpe                — start new pipeline from conversation
/rdf:r-vpe --resume       — resume interrupted pipeline from state file
...
- Sub-commands (/rdf:r-spec, /rdf:r-plan, /rdf:r-build) create their own task
  lists — do NOT create duplicate umbrella tasks for those stages
- Create Ship only after /rdf:r-build completes all phases — this keeps
...
## Stage 2: Design (invokes /rdf:r-spec)

Before invoking /rdf:r-spec, check docs/specs/ for existing specs. If a
...
Otherwise, invoke /rdf:r-spec with the synthesized problem statement as
the seed input. The user participates in brainstorming and design
questions as normal — VPE does not suppress or shortcut the /rdf:r-spec
workflow.

VPE's role during spec:
- Ensure the problem statement from intake is the starting context
- Let /rdf:r-spec handle all brainstorming, research, and spec writing
- After /rdf:r-spec completes, read the committed spec path
...
## Stage 3: Plan (invokes /rdf:r-plan)

Invoke /rdf:r-plan with the spec path. The user approves the plan as
normal — VPE does not suppress the /rdf:r-plan workflow.
...
## Stage 4: Build (invokes /rdf:r-build --parallel)

Invoke /rdf:r-build --parallel. The build command handles all phase
...
- Failures exist → present to user:
  "Build completed with failures:
   {failure summary from /rdf:r-build}
...
## Stage 5: Ship (invokes /rdf:r-ship)

Mark task "Ship" as in_progress.

Invoke /rdf:r-ship. The user confirms the PR as normal — VPE does not
suppress the /rdf:r-ship workflow.
...
- Max 4 exchanges during intake — escalate to /rdf:r-spec if more exploration needed
