...
- `--force` — delete existing `.rdf/governance/` and regenerate
  from scratch. Without this flag, /rdf:r-init refuses to run when
  governance already exists (directs to `/rdf:r-refresh` instead).
...
/rdf:r-init runs 5 phases in sequence:
...
If `.rdf/governance/` already exists (from a prior /rdf:r-init or
/rdf:r-refresh):

- **Without `--force`:** stop and direct the user to `/rdf:r-refresh`
...
> **Ready** — `/rdf:r-start` to begin | `/rdf:r-spec` to design | `/rdf:r-plan` to plan
```

**Pipeline-aware Ready line:** If an active plan or specs already exist,
adjust the hint:
- Specs exist, no active plan → `> **Ready** — `/rdf:r-plan` to create implementation plan`
- Active plan exists → `> **Ready** — `/rdf:r-build` to start phase 1`
- Nothing exists → `> **Ready** — `/rdf:r-start` to begin | `/rdf:r-spec` to design`
...
1. **NEVER modify existing convention files** — CLAUDE.md, AGENTS.md,
   ...
   .github/copilot-instructions.md are read-only inputs to /rdf:r-init.
...
4. If `.rdf/governance/` already exists with files AND `--force`
   ...
   - "Governance files already exist. To update, use `/rdf:r-refresh`."
   - "To regenerate from scratch, use `/rdf:r-init --force`."
   - Stop without modifying existing governance files.
   If `--force` IS set, delete `.rdf/governance/` and continue.

### Monorepo Behavior

1. `/rdf:r-init .` at the repo root produces governance for the dominant
   patterns across the whole repository.
2. `/rdf:r-init ./services/api` scopes Phases 2-3 to that subdirectory
...
- `/rdf:r-init` is for first-time governance generation. It refuses to
  run if `.rdf/governance/` already exists (see Error Handling #4).
- `/rdf:r-init --force` deletes existing governance and regenerates from
  scratch. Use when governance is stale, corrupt, or after major
  codebase restructuring that `/rdf:r-refresh` can't handle.
- `/rdf:r-refresh` is for updating governance after codebase changes.
...
If /rdf:r-init is run and produces governance files, running /rdf:r-init
again (without `--force`) will NOT modify anything — it detects
existing governance and directs the user to `/rdf:r-refresh`.
