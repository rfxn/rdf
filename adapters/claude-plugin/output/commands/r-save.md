...
to `/rdf:r-start` — save writes the journal, start reads it.
...
invoke `/rdf:r-util-mem-compact` in preview mode and carry its proposed
...
Max 50 entries — run `/rdf:r-util-mem-compact` to prune.
```
...
- `plan` → plan ready, `/rdf:r-build` next
- `build phase 3/7` → actively building
- `ship` → all phases done, `/rdf:r-ship` next
...
- Memory: >=180 lines → auto-run `/rdf:r-util-mem-compact` preview, show reduction
...
- `idle` → `Spec a feature with /rdf:r-spec, or plan directly with /rdf:r-plan`
- `spec` → `Spec ready — /rdf:r-plan to create implementation plan`
- `plan` → `/rdf:r-build to start phase 1`
- `build phase N/M` → `/rdf:r-build {N+1} to continue` or `/rdf:r-ship if complete`
- `ship` → `All phases done — /rdf:r-ship to release`
...
- Idempotent: running /rdf:r-save twice with no intervening work should
