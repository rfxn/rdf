...
- Note: "Governance not initialized. Proceeding with available context.
  Consider running `/rdf:r-init` for richer project understanding."
...
- Probe external/runtime unknowns directly — binary paths, protocol/auth
  ...
  checks or `/rdf:r-verify-claim`, and fold the results into the spec as facts
...
everything during `/rdf:r-plan`. The inventory table is the proof that
...
> Reviewed and approved. Run `/rdf:r-plan` in this same session to create the implementation plan.
```
