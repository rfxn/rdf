...
If governance doesn't exist, replace that cell with `none — /rdf:r-init`.
...
(written by `/rdf:r-save`); omit the segment otherwise.
...
```
> ▶ Governance {T}h old — `/rdf:r-refresh` | MEMORY.md {N}/200 — previewed compaction saves {M} lines | Context ~{N}% — fresh session or `/half-clone`
```

When MEMORY.md ≥180, run `/rdf:r-util-mem-compact` in preview mode and render the
...
- Memory size: >=180 lines → auto-run `/rdf:r-util-mem-compact` preview
...
not by `/rdf:r-start`; `/rdf:r-start` only surfaces the count on the last line above.
