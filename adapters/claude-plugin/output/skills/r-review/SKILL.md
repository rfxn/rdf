...
`/rdf:r-review [--challenge <file>] [--sentinel] [scope]`
...
- If `$ARGUMENTS` contains `--challenge`:
  ...
  - If no file path provided: report error "Usage: /rdf:r-review
...
- Read `.rdf/governance/index.md`
  - If governance index does not exist, warn: "No governance found.
    Review will use general best practices only. Run /rdf:r-init for
...
effort). Never pass a per-invocation model — model and effort live in the
agent definitions.
...
- To route each finding to a structured FIX / REBUT / DEFER response and
  record an audit trail, run `/rdf:r-review-answer` (advisory — no merge gate)
