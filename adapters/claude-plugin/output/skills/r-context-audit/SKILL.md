...
1. **Memory compact** — invoke `/rdf:r-util-mem-compact --apply`
...
1. **Weekly**: Run `/rdf:r-context-audit` to catch drift
2. **After adding skills/memory**: Run to verify impact
3. **Before major builds**: Run to ensure headroom
4. **After /rdf:r-init or /rdf:r-refresh**: Run to verify governance didn't bloat
