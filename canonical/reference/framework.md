...
**Plan status markers** (semantic states):
...
Bash calls, shared with subagents, kept by `--resume <id>` unless
`--fork-session`, renewed on `/clear`), else a minted UUIDv7. An explicit `RDF_SESSION_ID`
