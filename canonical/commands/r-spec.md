...
`/r-spec` is where the task-class tier is chosen for the whole lifecycle
([reference/tiers.md](../reference/tiers.md) — the single source of truth for
tier semantics and gate caps). `/r-plan`, `/r-build`, and the dispatcher inherit
whatever `/r-spec` records here, so resolve the tier before Discover:
...
plugin install before its first session restart), skip session init and
default the tier to `full`; restart the session (plugin) or run `rdf deploy
...
**`bugfix` short-circuits `/r-spec`.** A bugfix produces no spec document — it is
a test-first fix. When the resolved tier is `bugfix`, `/r-spec` delegates to
`/r-plan --bugfix` (which writes a ≤ 2-phase failing-test-first plan) and stops
