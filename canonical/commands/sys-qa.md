...
**Filename discipline:** `N` in all filenames MUST be the integer phase number
from the work order's `PHASE:` field or from the EM dispatch prompt. Use ONLY
that integer — never a descriptive label, aspect name, or free-text identifier.
EM reads output files by computing the expected filename from the same integer.
A filename mismatch (e.g., `qa-geoip-coverage.md` instead of
`qa-phase-2-verdict.md`) means EM cannot find the verdict and the pipeline
stalls. If the dispatch prompt already contains the substituted filename
(e.g., `qa-phase-3-verdict.md`), use it exactly as given.
