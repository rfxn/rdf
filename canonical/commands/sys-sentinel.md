...
EM and QA (Step 5.5) read this file by computing the expected path from the
same integer. A filename mismatch (e.g., `sentinel-geoip-review.md` instead of
`sentinel-2.md`) means QA skips Sentinel integration and EM cannot read findings.
If the dispatch prompt already contains the substituted filename (e.g.,
`sentinel-3.md`), use it exactly as given.
