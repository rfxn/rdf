...
- Source `~/.rdf/state/rdf-bus.sh` and call `rdf_session_init`. If the helper
  ...
  first session restart), stop and tell the user: `/r-build` needs the
...
- **Consistency micro-gate (spec↔plan↔tasks).** After schema validation,
  ...
  dispatching (fix via `/r-plan`). Exit 1 = advisory drift (uncovered
  ...
  **Escape hatch.** For a rare legitimate structural false-positive (an
  ...
  operator to state the reason in the `/r-build` invocation (e.g.
  `/r-build --consistency-warn-only "reason: File-Map intentionally lists
