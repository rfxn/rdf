# /r-review-answer — Reviewer-Feedback Routing
...
any unanswered MUST-FIX, but it does not block `/r-build`, `/r-ship`, or
merge. The value is the trail, not a gate.

## Invocation

```
/r-review-answer [<report-file>]
/r-review-answer --from <path>
```
...
- No argument: resolve the most recent session-scoped report —
  ...
  none, report "No review report found — run /r-review --sentinel first"
  and stop.
...
|---------|---------|-------------------|
| `FIX` | Addressed in the code | commit `<sha>` or `<path>:<line>`; for a current-state claim, back it with `/r-verify-claim` |
| `REBUT` | Finding is wrong or not applicable | the reason; for a current-state dispute, run `/r-verify-claim` and cite its PASS |
| `DEFER` | Valid but out of scope now | issue link, or a one-line reason plus the scope it belongs to |

Reuse `/r-verify-claim --from-finding <report>` to generate evidence for
...
- FINDING 2 [SHOULD-FIX(advisory-concern)] — <text>
  VERDICT: REBUT
  EVIDENCE: /r-verify-claim PASS — <one-line result>
...
- Flag any REBUT that carries no evidence (boilerplate "not applicable"):
  `⚠ N REBUT without evidence — cite a /r-verify-claim PASS or downgrade to DEFER`.
...
- Advisory only — does not gate `/r-build`, `/r-ship`, or merge; unanswered
  or boilerplate findings are surfaced, not enforced
- Every REBUT of a current-state claim should cite a `/r-verify-claim` PASS;
