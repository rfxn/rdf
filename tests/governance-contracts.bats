...
@test "engineer result declares a TDD_EVIDENCE section" {
    ...
    _no_contract agents/engineer.md 'TDD_EVIDENCE[^.]*(^|[^[:alnum:]_])(no longer|not required|optional|removed)([^[:alnum:]_]|$)'
}
...
@test "dispatcher emits NEEDS_CONTEXT when a DONE result lacks EVIDENCE" {
    ...
    # negation guard: bound to the contract verb — a bare negative word trips on
    # reinforcing prose ("... and the phase does not advance").
    _no_contract agents/dispatcher.md 'NEEDS_CONTEXT[^.]*(is|are) (no longer|not) (emitted|required|used)'
}
...
@test "r-review-answer is advisory — does not block build/ship/merge" {
    ...
    # negation guard: bound to subject+verb — the bare `blocks? (…)` branch tripped
    # on the reinforcing "never blocks merge" (explicit if/return — see the
    # security-floor test below for why bare `! grep` is unsafe)
    if grep -qiE '(/r-review-answer|it|this)[^.]*(is|becomes) a (blocking|hard) gate|must pass before (/r-build|/r-ship|merge)' "$f"; then
...
@test "dispatcher caps gates by tier, never upgrades" {
    ...
    _no_contract agents/dispatcher.md 'tier cap[^.]*(^|[^[:alnum:]_])(can|may|will|does)[^.]*upgrade([^[:alnum:]_]|$)'
}
...
@test "tier cap never drops the security pass on scope:sensitive" {
    ...
    # negation guard: no sentence disapplies the floor. Explicit `if ...; return 1`
    # rather than bare `! grep` — bats runs bodies under `set -e`, which exempts
    # `!`-negated commands, so a bare `!` never fails the test (SC2314, CI-gated).
    if grep -qE 'Security floor[^.]*(^|[^[:alnum:]_])(no longer|not|never|does not)([^[:alnum:]_]|$)[^.]*appl' "$d" "$t"; then
        echo "negation violated: a disclaimer sentence disapplies the security floor"
        return 1
    fi
    # order: the floor paragraph precedes the first tier bullet inside the Tier Cap
    # section. Each anchor gets its own `|| return 1` — chained with `&&` a missing
    # anchor makes the whole line a non-final `&&` list, which `set -e` exempts.
    ...
    [ -n "$cap" ] || { echo "missing anchor: ### Tier Cap in dispatcher.md"; return 1; }
    [ -n "$floor" ] || { echo "missing anchor: **Security floor in dispatcher.md"; return 1; }
    [ -n "$bullet" ] || { echo 'missing anchor: - `full` tier bullet in dispatcher.md'; return 1; }
    if ! [ "$cap" -lt "$floor" ]; then
        echo "order violated: ### Tier Cap ($cap) must precede **Security floor ($floor)"
        return 1
    fi
    if ! [ "$floor" -lt "$bullet" ]; then
        echo "order violated: **Security floor ($floor) must precede the first tier bullet ($bullet)"
        return 1
    fi
...
@test "r-spec Clarify precedes Brainstorm" {
    ...
    [ -n "$clarify" ] || { echo "missing anchor: ## Phase 1.5: Clarify in r-spec.md"; return 1; }
    [ -n "$brainstorm" ] || { echo "missing anchor: ## Phase 2: Brainstorm in r-spec.md"; return 1; }
    if ! [ "$clarify" -lt "$brainstorm" ]; then
        echo "order violated: Clarify ($clarify) must precede Brainstorm ($brainstorm)"
        return 1
    fi
    ...
    if grep -qE 'Clarify[^.]*(^|[^[:alnum:]_])(removed|eliminated|no longer exists|folded into Brainstorm)([^[:alnum:]_]|$)' "$f"; then
...
@test "dispatcher runs a mandatory end-of-plan sentinel for 3+ phase plans" {
    ...
    _no_contract agents/dispatcher.md 'End-of-Plan Sentinel[^.]*(^|[^[:alnum:]_])(optional|no longer mandatory|removed|not required)([^[:alnum:]_]|$)'
}
...
@test "dispatcher writes structured status to work-output after each phase" {
    ...
    _no_contract agents/dispatcher.md 'status (writes?|files?)[^.]*(^|[^[:alnum:]_])(no longer|optional|not required|removed)([^[:alnum:]_]|$)'
}
