...
**Architecture:** Each utility is a markdown skill file in `canonical/commands/` with the filename pattern `r-util-{subject}-{verb}.md`. The slash command is derived directly from the filename (`/r-util-mem-compact`). All utilities follow the `{subject}-{verb}` naming convention where subjects (`mem`, `doc`, `ci`, `lib`, `code`, `test`, `chg`) group related tools for autocomplete discovery. Content is ported from RDF 2.x commands with adaptations for the 3.0 governance model (governance index, just-in-time context loading, no hardcoded project paths).
