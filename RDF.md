...
- **RDF is authoritative:** The repo is the source of truth. `~/.claude/`
  (`agents/`, `skills/<name>/SKILL.md` per command, `scripts/`, `reference/`)
  and other tool-specific directories are deployment targets populated by
  `rdf generate`. Emergency edits to deployed copies are pulled back via
  `rdf sync`.
...
|   |
|   |-- commands/                    # 37 commands under /r- namespace
...
|   |   |-- r-vpe.md                 # Optional pipeline orchestrator
|   |   |-- r-save.md                # Session state sync
|   |   |-- r-tasks.md               # Task list status
|   |   |-- r-spec.md                # Specification authoring
...
|
|-- profiles/                        # 13 registered profiles (registry.json) + lite
...
|   |-- node/                        # Node.js (plain JavaScript) projects
...
|   |-- migration/                   # Cross-version migration
...
|
|-- adapters/                        # 5 adapters (Codex/Antigravity = composite, no own directory)
|   |-- claude-code/
|   |   |-- adapter.sh               # CC-specific generation logic
|   |   |-- agent-meta.json          # YAML frontmatter values per agent
|   |   |-- hooks/
|   |   |   +-- hooks.json           # CC hook event mapping
|   |   +-- output/                  # GENERATED by 'rdf generate claude-code' -> skills/<n>/SKILL.md, agents/, scripts/, reference/
|   |-- claude-plugin/
|   |   |-- adapter.sh               # Plugin generation: /r-X -> /rdf:r-X rewrite, plugin-root hooks
...
- Additional discipline profiles beyond the current 13 (`profiles/registry.json`)
